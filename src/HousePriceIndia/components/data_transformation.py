...
from src.HousePriceIndia.utils import save_object
...
class DataTransformation:
    ...
    def get_data_transformer_object(self, raw_path):
        ...
        try:
            ...
            X_columns = df.drop(columns=[target_column_name],axis=1).columns

            pipeline = Pipeline(steps = [
                ('scaler', StandardScaler())
            ])

            preprocessor = ColumnTransformer([("pipeline", pipeline, X_columns)])
            return preprocessor
